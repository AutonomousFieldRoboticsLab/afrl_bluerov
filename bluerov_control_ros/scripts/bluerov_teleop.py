...
class JoyStick(object):
    ...
    def convert_motor_intensities_to_pwms(self, intensities):
        ...
        for i in range(len(offset_motor_speeds)):
            ...
            offset_motor_speeds[i] = offset_motor_speeds[i] + \
                (intensities[i] * self.max_motor_speed) + offset
    ...
    def joy_cb(self, joy_msg):
        ...
        motor_intensities = self.convert_thrust_vector_to_motor_intensities(
            thurst)

        motor_intensities = [intensity * direction for intensity,
                             direction in zip(motor_intensities, self.directions)]
        ...
        self.light_value = np.clip(
            self.light_value, self.min_pwm, self.max_pwm)
    ...
    def convert_thrust_vector_to_motor_intensities(self, thrust_vector):
        ...
        normalized_intensities = [
            val / max_intensity for val in motor_intensities]
